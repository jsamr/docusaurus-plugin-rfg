...
import { createRequest, generateFaviconAsync } from './rfg';
import { RFGResponse } from 'rfg-api';
...
export interface RFGPluginOptions {
  icon: string;
  apiKey: string;
  name: string;
  id?: string;
...
const NAME = 'docusaurus-plugin-rfg';
...
const plugin: PluginModule = function plugin(
  ...
  function task__fillHeadTags(content: RFGResponse) {
    const html = content.favicon.html_code;
